...
export const getPatientHistory = async (req, res) => {
    try {
        ...
        const healthLogs = await HealthLog.find({ userId: patient._id }).sort({ date: -1 });  // changes needed here might be
        const documents = await Document.find({ userId: patient._id }).sort({ createdAt: -1 }); // changes needed here might be
